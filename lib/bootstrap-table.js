...
import locale from './locale/en-US.js'
...
class BootstrapTable {
  ...
  constructor ($table, options = {}) {
    ...
    this.$table = $table
    this.#clone = $table.cloneNode(true)
  ...
  initConstants () {
    ...
    this.constants.theme = module.theme
    this.constants.dataToggle = this.constants.html.dataToggle || 'data-toggle'

    // init iconsPrefix and icons
    const iconsPrefix = 'bi'
    ...
    opts.iconsPrefix = iconsPrefix
    opts.icons = Object.assign(icons, module.defaults.icons, opts.icons)
  ...
  initContainer () {
    ...
    const loadingTemplate = Utils.calculateObjectValue(
      this.options,
      this.options.loadingTemplate,
      [locale.formatLoadingMessage()]
    )
    const $container = document.createElement('div')
    $container.className = 'bootstrap-table bootstrap5'
    $container.innerHTML = `
    ...
    this.$tableContainer = $container.querySelector('.fixed-table-container')
    this.$tableHeader = $container.querySelector('.fixed-table-header')
    this.$tableBody = $container.querySelector('.fixed-table-body')
    this.$tableLoading = $container.querySelector('.fixed-table-loading')
    this.$tableFooter = this.$table.querySelector('tfoot')
    // checking if custom table-toolbar exists or not
    if (this.options.buttonsToolbar) {
      this.$toolbar = $('body').find(this.options.buttonsToolbar)
    } else {
      this.$toolbar = $container.querySelector('.fixed-table-toolbar')
    }
    this.$pagination = $container.querySelector('.fixed-table-pagination')

    this.$tableBody.append(this.$table)
    this.$container.after('<div class="clearfix"></div>')

    // this.$table.addClass(this.options.classes)
  ...
  initTable () {
    const columns = []
    const $header = this.$table.tHead?.[0] || this.$table.createTHead()

    this._headerTrClasses = []
    this._headerTrStyles = []

    for (const $tr of $header.rows) {
      const column = []
      ...
      if ($tr.className) {
        this._headerTrClasses.push($tr.className)
      }
  ...
  resetHeader () {
    ...
    this.#timeoutId = setTimeout(() => this.fitHeader(), this.$el.is(':hidden') ? 100 : 0)
  }

  fitHeader () {
    if (this.$el.is(':hidden')) {
      this.#timeoutId = setTimeout(() => this.fitHeader(), 100)
      return
    }
...
const module = {
  bootstrapTable(option, ...args) {
    let value

    const tables = document.querySelectorAll('table[data-toggle="table"]')

    for (const table of tables) {
      // const data = table.dataset
      new module.BootstrapTable(table)
    }
  },
  BootstrapTable,
  theme: Constants.THEME,
  VERSION: Constants.VERSION,
  defaults: BootstrapTable.DEFAULTS,
  columnDefaults: BootstrapTable.COLUMN_DEFAULTS,
  events: BootstrapTable.EVENTS,
  locales: BootstrapTable.LOCALES,
  methods: BootstrapTable.METHODS,
  utils: Utils
}
